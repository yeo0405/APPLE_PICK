...
class PoseSolver(BaseSolver):
    ...
    def fit(
        ...
        for epoch in range(start_epcoch, args.epoches):
            ...
            train_stats = train_one_epoch(
                ...
                writer=self.writer,
                progress_bar=args.yaml_cfg.get("progress_bar", False),
            )
    ...
    def val(
        ...
        test_stats, coco_evaluator = evaluate(
            ...
            self.device,
            report_pose=True,
            overlay_dir=self.cfg.yaml_cfg.get("overlay_dir"),
            overlay_limit=self.cfg.yaml_cfg.get("overlay_limit", 100),
            score_threshold=self.cfg.yaml_cfg.get("score_threshold", 0.25),
            pose_iou_threshold=self.cfg.yaml_cfg.get("pose_iou_threshold", 0.5),
        )
        ...
        if self.output_dir and dist_utils.is_main_process():
            with (self.output_dir / "log_test.txt").open("a") as f:
                f.write(json.dumps(log_stats) + "\n")
            with (self.output_dir / "evaluation_stats.json").open("w") as f:
                json.dump(log_stats, f, indent=2, allow_nan=False)
