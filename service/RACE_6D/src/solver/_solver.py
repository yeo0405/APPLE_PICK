...
class BaseSolver(object):
    ...
    def _setup(self, ):
        ...
        if decoder is not None and hasattr(self.criterion, 'set_pose_source'):
            self.criterion.set_pose_source(decoder)
    ...
    def train(self, ):
        ...
        self.train_dataloader = dist_utils.warp_loader(self.cfg.train_dataloader, \
            shuffle=self.cfg.train_dataloader.shuffle, seed=seed)
    ...
    def load_resume_state(self, path: str):
        ...
        #     print(state['lr_scheduler'])
        #     state['lr_scheduler']['milestones'] = Counter({30:1})
        #     print('LR Scheduler milestone changed to [30]')
            
        # if 'lr_warmup_scheduler' in state:
        #     del state['lr_warmup_scheduler']
        #     print('LR Warmup Scheduler state removed - using new configuration')
