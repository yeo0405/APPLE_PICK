...
from .pose_reporting import PoseEvaluationReporter
    ...
    **kwargs,
    ...
    for i, (samples, targets) in enumerate(
        metric_logger.log_every(
            data_loader, print_freq, header, progress_bar=progress_bar
        )
    ):
...
def evaluate(
    ...
    device,
    report_pose: bool = False,
    overlay_dir: str | None = None,
    overlay_limit: int = 100,
    score_threshold: float = 0.25,
    pose_iou_threshold: float = 0.5,
):
    """Run loss/COCO evaluation and optional detection-aware pose reporting."""
    model.eval()
    criterion.eval()
    ...
    pose_reporter = None
    if report_pose and dist_utils.is_main_process():
        pose_reporter = PoseEvaluationReporter(
            criterion,
            data_loader,
    ...
    for samples, targets in metric_logger.log_every(data_loader, 100, header):
        ...
        if pose_reporter is not None:
            pose_reporter.update(outputs, results, targets)
    ...
    if pose_reporter is not None:
        pose_stats = pose_reporter.summarize()
        stats.update(pose_stats)
        print("Pose stats:", pose_stats)
