...
def distance_box_iou(boxes1, boxes2):
    """Distance-IoU: IoU - ρ²(center, center_gt) / c²
    boxes in [x0, y0, x1, y1] format. Returns [N, M] pairwise matrix.
    """
    ...
    diag = ((rb - lt) ** 2).sum(dim=-1)  # [N, M]

    return iou - center_dist / diag.clamp(min=1e-6)
