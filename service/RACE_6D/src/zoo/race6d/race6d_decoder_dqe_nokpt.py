...
__all__ = ['RACE6DTransformer_DQE_NoKpt']
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            # Keypoints: auxiliary supervision head (additive refinement, decoupled from rot)
            kpt_embed = kpt_head[i](output)
            inter_ref_kpts = kpt_embed + ref_kpts_detach  # [B,N,2*num_keypoints]
            with torch.amp.autocast('cuda', enabled=False):
                rot_input = output.float()  # rot decoupled: no kpt concatenation
...
class RACE6DTransformer_DQE_NoKpt(nn.Module):
    ...
    def __init__(self,
        ...
        self.enc_rot_head = MLP(hidden_dim, hidden_dim, 6, 3, mlp_act)
        ...
        self.dec_kpt_head = nn.ModuleList([
            MLP(hidden_dim, hidden_dim, 2 * num_keypoints, 3, mlp_act) for _ in range(num_layers)
        ])
        ...
        self.dec_rot_head = nn.ModuleList([
            MLP(hidden_dim, hidden_dim, 6, 3, mlp_act) for _ in range(num_layers)
        ]) # rotation-6d (input: output only, NOT concatenated with kpt)
    ...
    def _get_decoder_input(self,
        ...
        with torch.amp.autocast('cuda', enabled=False):
            enc_rot_input = enc_topk_memory.float()  # rot decoupled: no kpt concatenation
