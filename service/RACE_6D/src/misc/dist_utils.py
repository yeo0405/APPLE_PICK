...
def warp_loader(loader, shuffle=False, seed=0):
    from functools import partial
    _worker_init = partial(worker_init_fn, seed=seed, epoch=0)

    if is_dist_available_and_initialized():
        sampler = DistributedSampler(loader.dataset, shuffle=shuffle, seed=seed)
...
def update_epoch_seed(base_seed: int, epoch: int):
    """Update random seeds for each epoch to vary data augmentation patterns
...
def worker_init_fn(worker_id, seed, epoch):
    """Initialize worker process seeds using global epoch

    Args:
        worker_id: Worker process ID
    """
    worker_seed = seed + epoch * 1000 + worker_id
