...
def get_coco_api_from_dataset(dataset):
    ...
    if isinstance(dataset, torchvision.datasets.CocoDetection):
        ...
        for ann in coco_gt.anns.values():
            if ann.get('ignore', False) or ann.get('visibility', 1.0) < 0.1:
                ann['iscrowd'] = 1
                n_ignored += 1
        if n_ignored > 0:
            coco_gt.createIndex()
            print(f'[get_coco_api_from_dataset] Marked {n_ignored} anns as iscrowd=1 '
                  f'(BOP convention: visibility<0.1 or ignore=True)')
        return coco_gt
